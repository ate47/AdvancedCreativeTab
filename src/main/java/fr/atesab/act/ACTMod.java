...
public class ACTMod {
	...
	public void onRenderTooltip(ItemTooltipEvent ev) {
		...
		if (!(mc.currentScreen instanceof GuiGiver || mc.currentScreen instanceof GuiModifier)
			...
			else if (mc.thePlayer != null && mc.thePlayer.isCreative())
				ev.getToolTip()
						.add(TextFormatting.GOLD + "[" + TextFormatting.YELLOW + I18n.format("gui.act.rightClick")
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
			...
			String s = (!ev.isShowAdvancedItemTooltips() ? ev.getItemStack().getItem().getRegistryName().toString()
					: "");
			...
			if (!ev.isShowAdvancedItemTooltips()) {
	...
	public void postInit(FMLPostInitializationEvent ev) {
		Item.REGISTRY.forEach(c -> {
			...
			if (i.equals(ADVANCED_ITEM))
				...
				NonNullList<ItemStack> sub = NonNullList.func_191196_a();
		...
		attributes.forEach(at -> registerStringModifier("attribute.name." + at.getAttributeUnlocalizedName(), "attributes",
				sm -> sm.setString(at.getAttributeUnlocalizedName())));
