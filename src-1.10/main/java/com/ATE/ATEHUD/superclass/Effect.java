...
public class Effect {
	...
	public static String[] type={
		getPotionItemField(0),getPotionItemField(1),getPotionItemField(2)
	},skin={
		"empty,0,0","water,0,0","mundane,0,0","thick,0,0","ankward,0,0",
		"night_vision,0,1","invisibility,0,1","leaping,1,1","fire_resistance,0,1","swiftness,1,1",
		"slowness,0,1","water_breathing,0,1","healing,1,0","harming,1,0","poison,1,1",
		"regeneration,1,1","strength,1,1","weakness,0,1","luck,0,0"
	},effects={
		"moveSpeed,1","moveSlowdown,2","digSpeed,3","digSlowDown,4","damageBoost,5","heal,6",
		...
		"weakness,18","poison,19","wither,20","healthBoost,21","absorption,22",
		"saturation,23","glowing,24","levitation,25","luck,26","unluck,27"
	};
	...
	public String getNBT(){
		return "{Id:"+Id+",Amplifier:"+Amplifier+",Duration:"+Duration+",ShowParticles:"+ShowParticles+",Ambient:"+Ambient+"}";
	}
	public static void defineEffects(){
		Item[] list=new Item[type.length];int j=0;
		for (int i = 0; i < type.length; i++) {
			String[] strl=type[i].split(",");
			if(strl.length==2){
				try {
					list[j]=(Item) Class.forName(strl[0]).getField(strl[1]).get(Item.class);j++;
				} catch (Exception e) {
					if(ModMain.AdvancedModActived)System.out.println("Fail to add Class/Field (Class or Field doesn't exist) "+strl[0]+"."+strl[1]);
				}
			} else {
				if(ModMain.AdvancedModActived)System.out.println("Fail to add Class/Field (data_size!=2) \""+type[i]+"\"");
			}
		}
		itm_type=new Item[j];
		...
		EffectType[] list1=new EffectType[effects.length];j=0;
		for (int i = 0; i < list1.length; i++) {
			...
			if(strl.length==2){
				try {
					...
					if(ModMain.AdvancedModActived)System.out.println("Fail to add effect "+strl[0]+" : "+strl[1]+" isn't an integer");
				}
			} else {
				if(ModMain.AdvancedModActived)System.out.println("Fail to add effect (data_size!=2) \""+effects[i]+"\"");
			}
		}
		...
		PotionSkin[] list2=new PotionSkin[skin.length];j=0;
		for (int i = 0; i < list2.length; i++) {
			String[] strl=skin[i].split(",");
			if(strl.length==3){
				boolean isLon=false,isStr=false;
				if(strl[1]=="1")isStr=true;
				if(strl[2]=="1")isLon=true;
				list2[j]=new PotionSkin(strl[0], isLon, isStr);
				j++;
			} else {
				if(ModMain.AdvancedModActived)System.out.println("Fail to add skin (data_size!=3) \""+skin[i]+"\"");
			}
		}
	 ...
	 * @param id The effect ID.
	 * @param duration The number of ticks before the effect wears off.
	 * @param amplifier The potion effect level. 0 is level 1. (max=126)
	 */
	...
	public Effect(int id,int duration,int amplifier,int ambient,int showParticles){
		Id=id;
		Duration=duration;
		if(amplifier>127)Amplifier=126;
		if(amplifier<-127)Amplifier=-126;
		Amplifier=amplifier;
		Ambient=ambient;
