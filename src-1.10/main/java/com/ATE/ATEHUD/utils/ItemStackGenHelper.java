...
public class ItemStackGenHelper {
    ...
    public static ItemStack setMaxEnchant(ItemStack itst, String name) {
    	for (int i = 0; i < com.ATE.ATEHUD.superclass.Enchantments.enchantments.length; i++) {
    		itst.addEnchantment(com.ATE.ATEHUD.superclass.Enchantments.enchantments[i].getEnchantment(), ModMain.MaxLevelEnch);
		}
    ...
    public static ItemStack setPushUpEnchant(ItemStack itst, String name) {
    	itst=getNBT(itst,"{display:{Name:\""+name+"\",Lore:[\""+I18n.format("act.minigame.pu.kopeople")+"\"]},AttributeModifiers:[{AttributeName:\"generic.attackDamage\",Name:\"generic.attackDamage\",Amount:0,Operation:1,UUIDLeast:8000,UUIDMost:4000},{AttributeName:\"generic.movementSpeed\",Name:\"generic.movementSpeed\",Amount:0.5,Operation:1,UUIDLeast:8000,UUIDMost:4000}]}");
    ...
    public static ItemStack setHyperProtectionEnchant(ItemStack itst, String name, boolean Thorns, boolean cheat) {
    	int lv=10;
    	if(cheat){lv=ModMain.MaxLevelEnch;}else{lv=10;}
    	itst.setStackDisplayName(name);
    ...
    public static ItemStack getCMD(String cmd, String name) {
    	...
    	if(ModMain.AdvancedModActived)System.out.println("Create commandblock with cmd="+cmd);
    	if(ModMain.AdvancedModActived)System.out.println(is.getTagCompound().toString());
    	is.setStackDisplayName(name);
    	return is;
    }
    public static ItemStack getNBT(Object item, String nbt) {
    	ItemStack is=null;
    	if(item instanceof Item)is = new ItemStack((Item)item);
    	if(item instanceof Block)is = new ItemStack((Block)item);
    ...
    public static List addTitle(List subItems,String text){
    	for (int i = 0; i < 4; i++) {subItems.add(dl);}
        subItems.add(setMeta(new ItemStack(Blocks.STAINED_GLASS_PANE),4,text)); //5
        for (int i = 0; i < 4; i++) {subItems.add(dl);}
    	return subItems;
    }
    public static List addBlank(List subItems,int blanks){
    	for (int i = 0; i < blanks; i++) {subItems.add(dl1);}
    	return subItems;
    }
    public static List addBlank(List subItems){
	 ...
	 * @param description Description the head
	 * @return
	 */
    public static ItemStack getCustomSkull(String url,String name,String description) {
        ...
        ItemStack head=getNBT(new ItemStack(Items.SKULL, 1, (short) 3), "{SkullOwner:{Id:\""+uuid+"\",Properties:{textures:[{Value:\""+(new String(encodedData))+"\"}]}}}");
	 ...
	 * @return
	 */
    public static ItemStack getCustomSkull(String url,String name){return getCustomSkull(url, name, "");}
    public static ItemStack getGive(String code) throws NumberInvalidException {
        ...
        Item itema = (Item)Item.REGISTRY.getObject(resourcelocation);
        Item item = null;
        if (itema == null)
        {
