...
public class SCHelp extends SubCommand {
	...
	public static ITextComponent getCommandPage(String commandName, int page, int maxPage) {
		...
		if (page != 1)
			c.appendSibling(new TextComponentString("<--").setStyle(new Style()
					.setClickEvent(
							new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/" + commandName + " " + (int) (page - 1)))
					.setColor(TextFormatting.RED)));
		else
			c.appendSibling(new TextComponentString("<--").setStyle(new Style().setColor(TextFormatting.DARK_GRAY)));
		c.appendSibling(new TextComponentString(" | ").setStyle(new Style().setColor(TextFormatting.GRAY)));
		if (page != maxPage)
			c.appendSibling(new TextComponentString("-->").setStyle(new Style()
					.setClickEvent(
							new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/" + commandName + " " + (int) (page + 1)))
	...
	public static ITextComponent getHelp(String commandName, SubCommand cmd, ICommandSender sender) {
		...
		case suggestCommand:
			...
			break;
		default:
			cs.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/" + commandName + " " + cmd.getName()))
					.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
							new TextComponentString(I18n.format("cmd.act.help.do"))
									.setStyle(new Style().setColor(TextFormatting.YELLOW))));
			break;
		}
		c.appendSibling(new TextComponentString("/" + commandName + " " + cmd.getSubCommandUsage(sender)).setStyle(cs));
		c.appendSibling(new TextComponentString(" : ").setStyle(new Style().setColor(TextFormatting.GRAY)));
		c.appendSibling(
				new TextComponentString(cmd.getDescription()).setStyle(new Style().setColor(TextFormatting.WHITE)));
		return c;
	}
	...
	public void processSubCommand(ICommandSender sender, String[] args, MainCommand mainCommand)
		...
		for (int i = page * elm; i < (page + 1) * elm && i < mainCommand.subCommands.size(); i++) {
			help.appendSibling(getHelp(mainCommand.getName(), mainCommand.subCommands.get(i), sender));
		}
		if (maxPage != 0)
			help.appendSibling(getCommandPage(mainCommand.getName() + " " + getName(), page + 1, maxPage + 1));
		ChatUtils.send(help);
